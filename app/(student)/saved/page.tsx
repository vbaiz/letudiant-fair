...
type SavedSubTab = 'swipe' | 'reels' | 'actualites' | 'salon';
...
export default function SavedPage() {
  ...
  // Article filters and selection
  const [articleFilterSort, setArticleFilterSort] = useState<'recent' | 'oldest' | 'a-z' | 'z-a'>('recent');
  ...
  const handleDeleteArticle = async (articleId: string) => {
    ...
    try {
      await deleteArticleFromWishlist(user.id, articleId);
    ...
    try {
      await Promise.all(
        Array.from(selectedArticles).map((articleId) =>
          deleteArticleFromWishlist(user.id!, articleId)
        )
      );
      setSavedArticles((prev) =>
        prev.filter((a) => !selectedArticles.has(a.id))
      );
  ...
  const getFilteredArticles = () => {
    ...
    // Apply date filter
    const now = new Date();
    if (articleFilterDate === 'today') {
      const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
      filtered = filtered.filter((a) => new Date(a.saved_at) >= todayStart);
    } else if (articleFilterDate === 'week') {
      const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
      filtered = filtered.filter((a) => new Date(a.saved_at) >= weekAgo);
    } else if (articleFilterDate === 'month') {
      const monthAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
      filtered = filtered.filter((a) => new Date(a.saved_at) >= monthAgo);
    }

    // Apply sorting
    if (articleFilterSort === 'recent') {
      filtered.sort((a, b) => new Date(b.saved_at).getTime() - new Date(a.saved_at).getTime());
    } else if (articleFilterSort === 'oldest') {
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        {activeTab === 'liens' && (
          <div>
            ...
            >
              {(['swipe', 'reels', 'actualites', 'salon'] as SavedSubTab[]).map((subTab) => (
                <button
                  key={subTab}
            ...
            {activeSubTab === 'actualites' && (
              <div>
                {articlesLoading ? (
                  ...
                  <p style={{ color: 'var(--le-gray-500)', fontSize: 14, textAlign: 'center', padding: '32px 16px' }}>
                    Aucun article sauvegardé. Cliquez sur "Enregistrer" pour ajouter des articles à votre collection.
                  </p>
                ) : (
                  <>
                    {/* Filters Section */}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 12 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                          <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--le-gray-700)' }}>Trier par:</label>
                          <select
                            value={articleFilterSort}
                            onChange={(e) => setArticleFilterSort(e.target.value as any)}
                            style={{
                              padding: '8px 12px',
                              borderRadius: 6,
                              border: '1px solid var(--le-gray-300)',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                          <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--le-gray-700)' }}>Date:</label>
                          <select
                            value={articleFilterDate}
                            onChange={(e) => setArticleFilterDate(e.target.value as any)}
                            style={{
                              padding: '8px 12px',
                              borderRadius: 6,
                              border: '1px solid var(--le-gray-300)',
                      ...
                      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
                        <input
                          type="checkbox"
                          checked={selectedArticles.size === getFilteredArticles().length && getFilteredArticles().length > 0}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedArticles(new Set(getFilteredArticles().map((a) => a.id)));
                            } else {
                              setSelectedArticles(new Set());
                            }
                          }}
                        ...
                        {selectedArticles.size > 0 && (
                          <span style={{ fontWeight: 600, color: 'var(--le-red)' }}>({selectedArticles.size} sélectionné{selectedArticles.size > 1 ? 's' : ''})</span>
                        )}
                      </label>
                    </div>
                      ...
                      {getFilteredArticles().map((article) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedArticles((prev) => new Set(prev).add(article.id));
                              } else {
                                setSelectedArticles((prev) => {
                                  const next = new Set(prev);
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, fontSize: 12, color: 'var(--le-gray-500)' }}>
                              ...
                              <span>•</span>
                              <span>{new Date(article.saved_at).toLocaleDateString('fr-FR')}</span>
                            </div>
                          </div>
                    ...
                    {savedArticles.length > 0 && (
                      <div style={{ textAlign: 'center' }}>
                        <button
                          onClick={() => router.push('/discover?tab=actualites')}
                          style={{
                            background: 'transparent',
                            color: 'var(--le-red)',
                            borderRadius: 6,
